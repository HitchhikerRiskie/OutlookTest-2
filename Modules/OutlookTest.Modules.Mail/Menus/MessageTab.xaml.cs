...
namespace OutlookTest.Modules.Mail.Menus
{
    ...
    public partial class MessageTab : ISupportDataContext, ISupportRichText
    {
        ...
        {
            if (e.NewValue == null || _updatingState)
                return;

            var font = (string)e.NewValue;

            RichTextEditor.Selection.ApplyFont(new RichTextFont(font));
        }
        ...
        void UpdateFontFamily(CharacterSettings settings) {
            ...
            _fontNames.Value = (settings.FontSettings.Ascii.HasValue && !string.IsNullOrWhiteSpace(settings.FontSettings.Ascii.Value.Name)) ? settings.FontSettings.Ascii.Value.Name : "Arial";
        }
